...
class EasyQuizManager:
    def __init__(self, screen, correct_sound, wrong_sound):
        self.screen        = screen
        self.correct_sound = correct_sound
        self.wrong_sound   = wrong_sound

        self.questions = [load_and_scale(f'ASSET/EASY/EASY ({i}).png') for i in range(1, 6)]
        self.answers   = ['A', 'B', 'A', 'A', 'B']
    ...
    def draw(self):
        # Always draw either the feedback or the question
        if self.feedback_image:
